...
def contact_query():
    ...
    database = get_db()
    buildings = database.execute('select  mudd, nwc, bulter, lerner, others, date from dailypass'
    ...
    for i in [mudd, nwc, lernerhall, bulter, others]:
        if i == 1:
            contact = database.execute('select username from dailypass where date = ？and ?=1', (date, i)).fetchone()
            num += len(contact) - 1
